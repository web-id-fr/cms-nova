<template>
    <DefaultField :field="field" :errors="errors" :show-help-text="showHelpText" :full-width-content="true" class="advanced-url-field-wrapper">
        <template #field>
            <SearchInput
                v-if="!isReadonly"
                @input="performSearch"
...
export default {
    ...
    methods: {
        ...
        selectResource(selectedResource) {
            this.selectedResource[this.currentLocale] = selectedResource;
            this.value[this.currentLocale] = selectedResource.value;

            this.$forceUpdate();
        },

        clearSelection() {
            this.selectedResource[this.currentLocale] = null;
            this.value[this.currentLocale] = null;
